...
generated quantities{
  array[total_obs_mrna] int<lower=0> mrna_sum_rep; // mrna posterior predictive transcript sum
  vector[total_obs_mrna] mrna_log_lik; // mrna log likelihood

for(n in 1:total_obs_mrna){
  // draw mrna log likelihood samples
  mrna_log_lik[n] = neg_binomial_2_lpmf(mrna_sum[n] | mrna_mean[n], phi_transform[n]);
}
  // collect mrna posterior predictive draws
