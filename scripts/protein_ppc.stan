...
generated quantities{
  array[total_obs_protein] real protein_bar_rep; // protein posterior predictive average
  vector[total_obs_protein] protein_log_lik; // protein log likelihood
  vector[total_obs_mrna] mrna_log_lik; // mrna log likelihood
  vector[total_obs] log_lik; // combine lod likelihood for both modalities

for(n in 1:total_obs_protein){
  // protein log likelihood draws 
  protein_log_lik[n] = normal_lpdf(protein_av[n] | protein_mean[n], protein_sd[n]);
}
for(n in 1:total_obs_mrna){
  // mrna log likelihood draws
  mrna_log_lik[n] = neg_binomial_2_lpmf(mrna_sum[n] | mrna_mean[n], phi_transform[n]);
}

  // combined vector of log likelihoods
  log_lik = append_row(protein_log_lik, mrna_log_lik);
  // collect protein posterior predictive draws 
